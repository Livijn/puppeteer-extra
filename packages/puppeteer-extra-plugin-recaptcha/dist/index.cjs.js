/*!
 * puppeteer-extra-plugin-recaptcha v3.3.3 by berstend
...
async function getSolutions(captchas = [], token, proxy = '') {
    const solutions = await Promise.all(captchas.map((c) => getSolution(c, token || '', proxy)));
    return { solutions, error: solutions.find((s) => !!s.error) };
}
async function getSolution(captcha, token, proxy) {
    const solution = {
